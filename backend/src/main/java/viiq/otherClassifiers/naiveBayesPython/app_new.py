...
output_path = sys.argv[1] #'/work/03629/rohitb/sklearn_nb/output_40k/'
...
pos_features = [t for t in index_feature if t > 0]
...
def _rank_candidates(features=None, candidates=None):
    ...
    for candidate in candidates:
        try:
            f_index = feature_index[candidate]
            #f_index = pos_features.index(candidate)
        #except ValueError:
        except KeyError:
            candidate_score[candidate] = negative_count
            negative_count -= 1
            continue
