...
TextboxList.Autocomplete = new Class({
	...
	search: function(bit){ 
		...
		var search = this.currentInput.getValue()[1];
                if (nodeEditFlag == 1){ setNodeEditSearchFlag(search, search.length);}
                else if (typeOrEntitySearchFlag == 1){ setTypeSearch(search, search.length);}
                  else if (typeOrEntitySearchFlag == 2) {setEntitySearch(search, search.length);} else {}
		...
		if (this.options.queryRemote){
                        if (nodeEditFlag == 1){
                           ...
                           jQuery("#entity-back").attr("disabled",true);} else{}
			   ...
			   this.values = this.searchValues[search];
                           ...
                           if (nodeEditFlag == 1){
                               ...
                               if(nodeEditFlag == 1){
                                  ...
                                  path = "http://[server_name]/orion_app/geteditnode?node="+nodeToBeEdited.nodeID+"&windownum="+0+"&windowsize="+nodeEditKeywordWindowSize+"&edges="+nodeEditEdges+"&keyword="+editNodeKeyword;
                               }
			       else if(typeOrEntitySearchFlag == 1){
                                  ...
                                  typeValue = -1;
                                  if (domain == 0)
                                   {  // all types based on keyword
                                     domainValue = -1;}
                                  else
                                  { // types  based on domain and keyword
                                     domainValue = jQuery("#domain-options option:selected").val();
                                  }
                                  path = "http://[server_name]/orion_app/gettypes?domain="+domainValue+"&windownum="+windownum+"&windowsize="+windowSize+"&keyword="+search;
                               }
                               else if (typeOrEntitySearchFlag == 2){
                                  ...
                                  if (domain == 0 && type == 0)
                                   {  // All entities based on keyword
                                     domainValue = -1;
                                     typeValue = -1;}
                                   
                                  else if (domain == 1)
                                  { // Entities based on domain and keyword
                                     domainValue = jQuery("#domain-options option:selected").val();
                                     typeValue = -1; 
                                  } else if (type == 1)
                                  ...
                                  path = "http://[server_name]/orion_app/getentities?domain="+domainValue+"&windownum="+windownum+"&windowsize="+windowSize+"&type="+typeValue+"&keyword="+search;
                               }
				   ...
				   for (var i =0; i < data1.length; i++){
					...
					that.values = temp;
                                        if (nodeEditFlag == 1){
                                            ...
                                            jQuery("#node-edit-entity-options").empty();
                                            if (temp.length < 19){
                                            jQuery("#node-edit-entity-options").attr('size',temp.length); nodeEditOptionsSize = temp.length;}
                                            else{
                                              nodeEditOptionsSize = 19; }
                                            ...
                                            jQuery("#type-options").empty();
                                            if (temp.length < 19){
                                            jQuery("#type-options").attr('size',temp.length); typeOptionsSize = temp.length;}
                                            else{
                                              typeOptionsSize = 19; }

                                             for (var i = 0; i < temp.length; i++) {
                                                    jQuery("#type-options").append('<option value="'+temp[i][0]+'" id="type-value-1">'+temp[i][1]+'</option>');
                                             };
                                        }
                                        else if (typeOrEntitySearchFlag == 2){
                                            setKeywordEntityCount(keywordEntityCount+1);
                                            ...
                                            jQuery("#entity-options").empty();
                                            if (temp.length < 19){
                                            jQuery("#entity-options").attr('size',temp.length); entityOptionsSize = temp.length;}
                                            else{
                                              entityOptionsSize = 19; }

                                             for (var i = 0; i < temp.length; i++) {
                                                    jQuery("#entity-options").append('<option value="'+temp[i][0]+'" id="entity-value-1">'+temp[i][1]+'</option>');
                                             };
                                          }
	...
	addCurrent: function(){
		...
		if (b){
			...
			b.inject($(this.currentInput), 'before');
                        ...
                        //jQuery("#type-options").empty();
                        //jQuery("#entity-options").empty();
                        //var path = "http://[server_name]/orion_app/gettypes?name=-38";//+value[0];
                        //var data1 = value[0];
                        //var temp=[];
                        //var returnVar=[];
                        //jQuery.ajax({
