...
function nodeNameText(selected_node, context){
  if(context == 1){
    ...
    jQuery('#modal-text').append("<h4>Instructions</h4>"+
                        "<ul>"+
                          "<li>Edge labels are sorted alphabetically in the <b>Edge Label</b> dropdown box. Choose the appropriate label to use from the list.</li>"+
                          "<li>Click the Select button to apply the selected edge label.</li>"+
                        "</ul>");
  }
...
// $('#queryConstructSVG').on('click', function(){
...
document.getElementById('entry_1433407723').value = endTimeVar;
document.getElementById('entry_938661300').value = diff;
});
    ...
    type:"GET",
     ...
     success: function(data) {
              if  (data !=null){
                  ...
                  document.getElementById('entry_2019824185').value = query_Id;
               }else{
