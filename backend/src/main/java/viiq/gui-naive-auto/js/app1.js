...
jQuery(document).ready(function(){
  ...
  getRequest("http://[server_name]/orion_app/greeting", domain);
  // Initial GET request to get all Entity suggestions.
  getAllEntities(entities);
  getAllTypes(types);
  jQuery("#wrapNew").toggleClass('active');
           ...
           .on('mouseover', function(d){
             ...
             if (d.greyflag == 0 && d.flag == 1){
               ...
               if (totalCount > 1){
                   ...
                   resultData.length = 0;
                   getRequest("http://[server_name]/orion_app/getnodetypevalues?node="+nodeMouseOver.nodeID+"&edges="+nodeEdgesMouseOver, resultData);
  ...
  g1.append('svg:circle')
    ...
    .on('dblclick', function(d){
         ...
         if (dblClickValid == true){
             ...
             getRequestWithLoader("http://[server_name]/orion_app/geteditnode?node="+nodeToBeEdited.nodeID+"&windownum="+0+"&windowsize="+windowSize+"&edges="+nodeEditEdges+"&keyword="+editNodeKeyword);
         }
    })
    .on('mousedown', function(d) {
          nodeclick = true;
...
function getExamples(link)
{
    ...
    setExamplesFlag();
    response.length = 0;
    getRequest("http://[server_name]/orion_app/getexamples?edge="+edgeId+"&source="+sourceNodeId+"&object="+targetNodeId, response);
    setExamplesFlag();
    animatedExamples();
    addEdgeButtonEdgeAdded = false;
...
function animatedExamples(){
    ...
    for (var i = 0; i < response.length; i++)
    {
     ...
     jQuery("#active-examples").append('<li class="li-animation"><b>Click</b> on Reverse Role button to reverse the role of source and destination of the edge.</li>');
     jQuery("#active-examples").append('<li class="li-animation"><b>Click</b> on the selected edge to remove souce type and destination type of edge.</li>');
...
function mousedown() {
  if (clearButtonFlag == 1 || usefulTipsFlag == 1 || edgeTypesFlag == 1 || settingsFlag == 1){
      if (clearButtonFlag == 1){
          clearButtonFlag = 0;
          jQuery("#wrap5").toggleClass('active');
      }
  ...
  }else if (usefulTipsFlag ==0 && question_flag == 2){ 
      usefulTipsFlag = 1;
      jQuery("#useful-tips").toggleClass('active');
      jQuery("#wrap2").toggleClass('active');
      jQuery("#wrapNew").hide();
      jQuery("#question_footer").css({visibility:"visible", opacity: 0.0}).animate({opacity: 1.0},500);
      jQuery("#question_Text_Footer").html(query_Question);
      startDateVar = new Date();
      startTimeVar = startDateVar.getTime();
      question_flag = 1;
      if (partialGraph.length<=0 && rejectedGraph.length<=0){
        document.getElementById('entry_1836649445').value = startTimeVar;
      }
      // question_flag = 0;
  }
  else{
  ...
  if (nextButtonClick) {
    ...
    {   
        ...
        // question_flag = 1;
        // if (partialGraph.length<=0 && rejectedGraph.length<=0){
        //   document.getElementById('entry_1088474550').value = startTimeVar;
        // }
...
jQuery("#node-edit-entity-back").click(function(){
    ...
    nodeEditWindowNo--;
    editList.length = 0;
    getRequest("http://[server_name]/orion_app/geteditnode?node="+nodeToBeEdited.nodeID+"&windownum="+nodeEditWindowNo+"&windowsize="+windowSize+"&edges="+nodeEditEdges+"&keyword="+editNodeKeyword, editList);
...
function animatedHelp(){
  //jQuery("#possible-actions").effect("highlight", 1000);
   // if ( possibleActionsColor == 0){
       // document.getElementById('possible-actions').style.backgroundColor="#F2F5A9";
     //   possibleActionsColor = 1;
   // }
   // else {
        //document.getElementById('possible-actions').style.backgroundColor="#F6E3CE";
     //   possibleActionsColor = 0;
   //}
  //document.getElementById('active-help').style.paddingRight="10px";
  //document.getElementById('active-help').style.textAlign="justify";
  if (nodes.length == 0 && links.length == 0) {
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the canvas to add a new node.</li>');
  }else if (checkConnected(nodes, links)) {
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Add</b> an edge between the newly added node and an existing node in the graph, by holding down the Shift key, clicking on one node and dragging the mouse pointer to the other.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Delete</b> the unconnected node.</li>');
  }else if(suggestions && !suggestionPicked && subSuggestionsInProgressFlag == false && exampleReverseRoleFlag == false){
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on a grey node to add it and its incident edge to the query graph.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the grey edge to select it and its corresponding grey node, or display the other occurrences of the grey edge, if any.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the empty canvas to ignore the automatic suggestions and add a new node.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on Refresh Suggestions to get a new set of edge recommendations.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Ignore</b> suggestions, close new node dialog box, view source type and destination type or edit node.</li>'); <!-- explain?-->
  }else if(suggestions && suggestionPicked && subSuggestionsInProgressFlag == false && exampleReverseRoleFlag == false){
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on other grey nodes to be included in the query graph.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the grey edge to select it, or click on a grey edge to display the other occurrences of the grey edge.</li>'); <!-- explain, diff betw this and previous 2nd-->
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the empty canvas to add the selected nodes and edges to the query graph while ignoring the unselected grey nodes, and display new suggestions.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on selected nodes (in blue) to unselect them.</li>');
  }else if(suggestions && SugPickdPerGreyLinkCount == 0 && greyLinkPickdPerSubSuggestion == 0  && subSuggestionsInProgressFlag == true){
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on a grey node to add it and its incident edge to the query graph.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on a grey edge to add it to the query graph, or on a grey node to add the node and its incident edge to the query graph.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the empty canvas to ignore the various unselected occurrences nodes and edges in grey, and go back to original set of suggestions.</li>');
  }else if(suggestions && (SugPickdPerGreyLinkCount > 0 || greyLinkPickdPerSubSuggestion > 0)  && subSuggestionsInProgressFlag == true){
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on other grey nodes to be included in the query graph.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on a grey edge to add it to the query graph, or on a grey node to add the node and its incident edge to the query graph.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the empty canvas to display original unselected suggestions in grey and the selected suggestions in blue.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on selected nodes to unselect them. Click on selected edge (in blue) between two orange nodes to unselect it.</li>');
  }
    ...
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Double</b> click on an orange node to edit its value.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on an edge to view its source type, object type.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on Refresh Suggestions to get a new set of edge recommendations.</li>');
  }
  else if (exampleReverseRoleFlag == true && !suggestions && addEdgeButtonEdgeAdded == false){
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on Add Edge button to add other instances of the selected edge, connected to the nodes at the two ends of the selected edge.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on Reverse Role button to reverse the role of source and destination of the edge.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the selected edge to remove souce type and destination type of edge.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on Refresh Suggestions to remove source type and destination type of edge and display new suggestions.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on empty canvas to remove source type and destination type of edge.</li>');
  }
  else if (suggestions && !suggestionPicked && exampleReverseRoleFlag == true && addEdgeButtonEdgeAdded == true){
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on a grey node to add it and its incident edge to the query graph.</li>');
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on the empty canvas to ignore the automatic edge suggestions and add a new node.</li>');
  }
  else if (suggestions && suggestionPicked && exampleReverseRoleFlag == true && addEdgeButtonEdgeAdded == true){
    jQuery("#active-help").empty();
    jQuery("#active-help").append('<li class="li-animation"><b>Click</b> on other grey nodes to be included in the query graph.</li>');
...
function postRequest(data, mode){
        ...
        url: "http://[server_name]/orion_app/greeting",
        data: jsonstr,
...
jQuery("#wrapNew-x-button").click(function(){
  if (question_flag == 2){
      usefulTipsFlag = 1;
      jQuery("#wrap2").toggleClass('active');
      jQuery("#useful-tips").toggleClass('active');
      jQuery("#wrapNew").hide();
      //jQuery("#question_footer").css({"visibility","visible"},{"opacity":"0.0"}).animate({"opacity","1.0"},2000);
      jQuery("#question_footer").css({visibility:"visible", opacity: 0.0}).animate({opacity: 1.0},500);
      //loadQuestion("http://[server_name]/orion_app/getnewquery");
      jQuery("#question_Text_Footer").html(query_Question);
      question_flag = 1;
  }
...
document.getElementById("entry_780852194").value = getGraphStringUserStudy(partialGraph);
document.getElementById("entry_598245090").value = getGraphStringUserStudy(rejectedGraph);
document.getElementById("entry_2100900924").value = no_Of_Iterations;
      }
