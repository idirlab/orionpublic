...
function mousedown() {
  if (clearButtonFlag == 1 || usefulTipsFlag == 1 || edgeTypesFlag == 1 || settingsFlag == 1){
      ...
      if (usefulTipsFlag == 1){
          usefulTipsFlag = 0;
          // time initialisation flag is used here to check for multiple clicks on the SVG.
          if(timeInitialisationFlag == 0){
            ...
            if (partialGraph.length<=0 && rejectedGraph.length<=0){
             document.getElementById('entry_1924013374').value = startTimeVar;
...
function collectDumpVariables(){
  document.getElementById("entry_1139604257").value = getGraphStringUserStudy(partialGraph);
  document.getElementById("entry_2035298029").value = getGraphStringUserStudy(rejectedGraph);
  document.getElementById("entry_1050483280").value = no_Of_Iterations;
