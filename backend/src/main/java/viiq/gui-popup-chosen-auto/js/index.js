...
//rudresh
var query_Question; // defined Global for maintaining common instance of the question.
//rudresh
...
$(document).ready(function(){
  // 1) this is the code where the question is loaded into the UI. This should be the only place where the question is loaded.
  // 2 ) Trying to load the query again will load a new random query!
...
document.getElementById('entry_1051255673').value = endTimeVar;
document.getElementById('entry_1858089459').value = diff;
});


// Ajax Function to do a get request for the random query!
// Data gets loaded into the question wrap ["#wrapNew -> question_Text-id"]
  ...
  jQuery.ajax({
    type:"GET",
     ...
     success: function(data) {
              if  (data !=null){
                  var array_Split = [];
                  array_Split = data.split("|");
                  query_Id = array_Split[0];
                  query_Question = array_Split[1];
                  jQuery("#question_Text").html(query_Question);
                  document.getElementById('question_footer').value = query_Question;
                  document.getElementById('entry_1458728843').value = query_Id;
               }else{
                 alert("No Questions retrived from the server");
               }
           }});
}
